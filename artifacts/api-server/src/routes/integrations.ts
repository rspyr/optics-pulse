...
router.post("/integrations/service_titan/backfill", requireRole("super_admin", "agency_user"), async (req, res) => {
  ...
  if (result.error) {
    ...
    // backfill / recompute-revenue cancel contract) so the UI shows a
    // "cancelled" toast instead of a misleading "backfill failed" error. Rows
    // already synced are kept (the sync log row is finalized as `cancelled`).
    if (result.error === "cancelled") {
      res.json({ success: true, cancelled: true, ...result });
      return;
    }
