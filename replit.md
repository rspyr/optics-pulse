...
- **Database:** PostgreSQL is the primary database, interfaced via Drizzle ORM. Database migrations run automatically on API server startup via two complementary systems: (1) `schema-migrations.ts` executes Drizzle-generated SQL files from `lib/db/drizzle/*.sql`, tracking applied tags in a dedicated `_applied_migrations` table (on first run it baselines every existing file as applied except a hardcoded bootstrap-pending list, so introducing the runner is non-destructive); (2) `one-time-migrations.ts` runs imperative data backfills tracked in `one_time_migrations`. `drizzle-kit push` is NEVER used in production — it is destructive when the journal drifts, and is reserved for local dev only via `pnpm --filter @workspace/db push`. The api-server build (`artifacts/api-server/build.ts`) copies `lib/db/drizzle/*.sql` into `dist/drizzle/` so the runner can resolve migration files from the bundled artifact. The post-merge hook (`scripts/post-merge.sh`) only runs `pnpm install --frozen-lockfile`; all schema application happens on server startup. Lazy initialization patterns are used to seed default data for new tenants.
