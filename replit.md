...
- **Lead Auto-Assignment:** All lead creation paths (webhook ingest, manual creation, Google Sheets ingest, sheet sync, demo generator) automatically assign leads to CSRs via round-robin routing. The shared `services/round-robin.ts` module powers all assignment. When routing config exists, it uses the cascade order, pause schedules, and pass-back rules. When no routing config exists, it falls back to distributing leads evenly across active client CSRs (client_user/client_admin roles) based on total assignment counts. A startup backfill iterates through any unassigned leads using the same logic.
