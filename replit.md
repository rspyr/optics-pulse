...
- **Sync-Process-Purge Pipeline:** After ServiceTitan jobs sync, the system immediately runs: (1) customer contact/address enrichment, (2) lead matching (stores permanent `lead_id` FK on jobs before PII purge), (3) reconciliation (attribution matching), (4) invoice sync. The PII purge (24h) wipes phone/email/name/stJobId but `lead_id` linkages survive. The 6h scheduled reconciliation remains as a fallback safety net.
