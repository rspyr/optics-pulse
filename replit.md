...
- **Leads HUD:** A gamified interface for lead management with real-time queues, quick actions, disposition logging, and performance statistics. Lead types are now seeded from the `funnel_types` table and the demo lead generator uses DB-backed funnel types per tenant. Communication routing supports CallRail click-to-call, Podium call/text, and native fallback — configured per-tenant via `communicationConfig` JSONB on the tenants table. Call/text actions from the HUD route through `POST /leads/:leadId/call` and `POST /leads/:leadId/text` endpoints, auto-logging attempts with method/platform tracking in `call_attempts`.
