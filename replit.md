...
- **Database:** PostgreSQL is the primary database, interfaced via Drizzle ORM. Database migrations are handled as one-time scripts in `artifacts/api-server/src/services/one-time-migrations.ts` and run **before** the API Server starts accepting traffic, so production stays in sync with the shipped schema on every deploy. Any schema change in `lib/db/drizzle/*.sql` that needs to reach production must also be mirrored as an idempotent entry in the `migrations` array there. Lazy initialization patterns are used to seed default data for new tenants.
