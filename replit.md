...
- **Database:** PostgreSQL is the primary database, interfaced via Drizzle ORM. On every startup — **before** the server accepts traffic — the API Server runs data-aware one-time migrations from `artifacts/api-server/src/services/one-time-migrations.ts`. Each entry is an idempotent, hand-written SQL block tracked in `_one_time_migrations` so it runs exactly once per environment. Any schema change in `lib/db/src/schema/` that must reach production **must** be mirrored as an entry here (ADD COLUMN IF NOT EXISTS, backfill, SET NOT NULL, etc.). We deliberately do not run `drizzle-kit push --force` against production at startup: `--force` is willing to execute destructive SQL like `TRUNCATE` when adding a NOT NULL column to a populated table, which is unsafe for production. `scripts/post-merge.sh` still runs `pnpm --filter db push` for the dev DB where data loss is acceptable. Lazy initialization patterns are used to seed default data for new tenants.
