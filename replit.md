...
- **Chat Analytics (Ask Your Data):** AI-powered natural language data querying using Gemini AI (gemini-3-pro-preview via Replit AI Integrations). Two-step pipeline: (1) AI generates structured JSON query plan, (2) tenant-scoped Drizzle ORM executor runs safe queries across all 15 tables, (3) AI generates streaming natural language answer via SSE. Arrow.js reactive templates render dynamic visualizations (bar, horizontal-bar, table, trend-line, pie, list, number/KPI). Strict tenant data isolation — tenantId always derived from authenticated session, never exposed to AI or accepted from request params. Chat drawer available on all authenticated pages via AppLayout. Saved questions per user/tenant. Key files: `chat-analytics.ts`, `data-query-executor.ts`, `chat.ts` (route), `chat-drawer.tsx`, `dynamic-visualization.tsx`. Dependencies: `@workspace/integrations-gemini-ai`, `@arrow-js/core`.
